...
const PlaylistPage: React.FC = () => {
  ...
  const [trackToAddId, setTrackToAddId] = useState<string | undefined>(undefined)
  ...
  const handleTapPlaylist = useCallback((id: string) => {
    if (!trackToAddId) {
      return
    }

    addTrackToPlaylist({ playlistId: id, songId: trackToAddId })
      .finally(() => {
        setPopupShowing(false)
        setTrackToAddId(undefined)
      })
  }, [myPlaylists, trackToAddId, setPopupShowing, setTrackToAddId, addTrackToPlaylist])
  ...
  return (
    <>
      <PlaylistPageContainer>
        <PlaylistHeader>
          {playlist.imageUrl && <PlaylistImage src={playlist.imageUrl} alt={playlist.name} />}
      ...
      {isPopupShowing && (
        <div style={{ position: 'fixed', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 100000 }}>
          <div 
            style={{ 
